...
#define GL_MULTISAMPLE_ARB 0x809D
    ...
    /* Create the wireframne shader */
    _wireframeShader = new OpenGLSolidColorShader();
    if (_wireframeShader == NULL) {
        log("ERROR allocating new SolidColorShader\n");
        return false;
    }

    if (_wireframeShader->init() == false) {
        log("ERROR initializing wireframe shader\n");
        return false;
    }
