...
int main(int argc, char**argv)
{
    ...
    Light light(glm::vec3(4.0, 4.0, 5.0),
	...
	renderTargetFXAA2->setClearColor(1.0, 1.0, 1.0, 0.0);
    ...
    glm::vec4 pos( 161.8, 179.3, 200.7, 1);
    _camera->setPosition(pos);
    _camera->rotatePitch(18.45);
    _camera->rotateYaw(321.4);

	/* Create a toon shader for the geometry */
	shader = Shader::New();

	std::string error;
	if (shader->loadVertexShader("data/shaders/lighting/toon.vert", error) == false) {
		printf("ERROR compiling vertex shader: %s\n", error.c_str());
		return 1;
	}
	if (shader->loadFragmentShader("data/shaders/lighting/toon.frag", error) == false) {
	...
	while (true)
	{
        ...
        if (_unboundFPS == true || (renderElapsedMs + totalAvgTime*jitterAdj >= dueTime)) {
            ...
            renderTargetFXAA2->blit(0, 0, width, height);
            console.blit();
