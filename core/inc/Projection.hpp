...
class Projection
{
	public:
		 ...
		 */
		void setFov(float fov)       { _fov    = fov;    _valid = false; }
		void setWidth(float width)   { _width  = width;  _valid = false; }
		void setHeight(float height) { _height = height; _valid = false; }
		void setNear(float pNear)    { _near   = pNear;  _valid = false; }
		void setFar(float pFar)      { _far    = pFar;   _valid = false; }
